...
function AmenidadCard({ a, onClick }: { a: Amenidad; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="group relative aspect-square rounded-2xl overflow-hidden shadow-card hover:shadow-caribe transition-all"
    >
      {a.imagen ? (
        <>
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-caribe-dark/90 via-caribe/30 to-transparent" />
        </>
      ) : (
        // Fallback elegante para amenidades sin foto
        <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-caribe to-caribe-dark p-4 group-hover:from-caribe-dark group-hover:to-caribe transition-all">
          <Icon className="w-14 h-14 text-turquesa mb-3" />
          <p className="text-white/60 text-xs uppercase tracking-widest">Render próximo</p>
        </div>
      )}
      <div className="absolute bottom-0 left-0 right-0 p-3 text-left">
        <p className="font-display text-white text-base md:text-lg leading-tight">{a.nombre}</p>
      </div>
    </button>
  );
}
...
export function Amenidades() {
  ...
  return (
    <section id="amenidades" className="py-24 px-6 bg-gradient-to-b from-crema to-turquesa-light">
              ...
              <div className="aspect-video relative bg-caribe">
                {selected.imagen ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-caribe to-caribe-dark p-6">
                    ...
                    <p className="text-white/70 text-sm uppercase tracking-widest">Render próximamente</p>
                  </div>
                )}
              </div>
              <div className="p-6">
                <h3 className="font-display text-caribe text-2xl mb-2">{selected.nombre}</h3>
                <p className="text-carbon">{selected.descripcion}</p>
                <p className="text-xs text-carbon/50 italic mt-4">
